...
|--------|---------|---------------|
| `scripts/validate_config.py` | Config validation + TOML parser + shared helpers | `parse_simple_toml()` :47, `validate_project()` :274, `load_config()` :451, `gh()` :22, `gh_json()` :32, `extract_sp()` :576, `get_team_personas()` :482, `get_milestones()` :510, `get_base_branch()` :534, `get_prd_dir()` :540, `get_test_plan_dir()` :549, `get_sagas_dir()` :558, `get_epics_dir()` :567, `get_story_map()` :609, `extract_story_id()` :634, `kanban_from_labels()` :643, `find_milestone()` :658, `warn_if_at_limit()` :688, `list_milestone_issues()` :677, `detect_sprint()` :622 |
