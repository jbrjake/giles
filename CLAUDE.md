...
|--------|---------|---------------|
| `scripts/validate_config.py` | Config validation + TOML parser + shared helpers | `parse_simple_toml()` :43, `validate_project()` :213, `load_config()` :390, `gh()` :23, `gh_json()` :33, `extract_sp()` :515, `get_team_personas()` :421, `get_milestones()` :449, `get_base_branch()` :473, `get_prd_dir()` :479, `get_test_plan_dir()` :488, `get_sagas_dir()` :497, `get_epics_dir()` :506, `get_story_map()` :544 |
| `scripts/sprint_init.py` | Auto-detect project → generate sprint-config/ | `ProjectScanner.scan()` :459, `ConfigGenerator.generate()` :793, `_inject_giles()` :674, `generate_definition_of_done()` :743, `generate_history_dir()` :748, `detect_prd_dir()` :341, `detect_test_plan_dir()` :358, `detect_sagas_dir()` :367, `detect_epics_dir()` :376, `detect_story_map()` :385, `detect_team_topology()` :398 |
| `scripts/sprint_teardown.py` | Safe removal of sprint-config/ | `classify_entries()` :20, `main()` :370 |
| `skills/sprint-setup/scripts/bootstrap_github.py` | Create labels/milestones on GitHub | `create_persona_labels()` :78, `_collect_sprint_numbers()` :91, `create_static_labels()` :171, `create_epic_labels()` :200, `create_milestones_on_github()` :211, `main()` :252 |
...
| `skills/sprint-run/scripts/sync_tracking.py` | Reconcile local tracking ↔ GitHub | `sync_one()` :206, `create_from_issue()` :253 |
| `skills/sprint-run/scripts/update_burndown.py` | Update burndown from GitHub milestones | `write_burndown()` :69, `update_sprint_status()` :108 |
...
| `scripts/sprint_analytics.py` | Sprint metrics (velocity, review rounds, workload) | `compute_velocity()` :61, `compute_review_rounds()` :98, `compute_workload()` :146, `format_report()` :169, `main()` :204 |
| `skills/sprint-monitor/scripts/check_status.py` | CI + PR + milestone + drift check | `check_ci()` :46, `check_prs()` :102, `check_milestone()` :178, `check_branch_divergence()` :232, `check_direct_pushes()` :270 |
...
| Modify config validation | Edit `scripts/validate_config.py` — `_REQUIRED_FILES` :186, `_REQUIRED_TOML_KEYS` :199 |
