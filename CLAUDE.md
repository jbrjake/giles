...
| `scripts/kanban.py` | Kanban state machine — transitions, assign, update, sync, status | `TRANSITIONS` §kanban.TRANSITIONS, `validate_transition()` §kanban.validate_transition, `check_preconditions()` §kanban.check_preconditions, `do_transition()` §kanban.do_transition, `do_assign()` §kanban.do_assign, `do_update()` §kanban.do_update, `do_sync()` §kanban.do_sync, `do_status()` §kanban.do_status, `find_story()` §kanban.find_story, `atomic_write_tf()` §kanban.atomic_write_tf, `lock_story()` §kanban.lock_story, `lock_sprint()` §kanban.lock_sprint |
