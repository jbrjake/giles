...
# ---------------------------------------------------------------------------
# P5-39: verify_line_refs tests
# ---------------------------------------------------------------------------

import tempfile
from verify_line_refs import extract_refs, find_symbol_line, verify_ref


class TestExtractRefs(unittest.TestCase):
    """P5-39: extract_refs correctly parses doc reference patterns."""

    def test_table_row_pattern(self):
        doc = tempfile.NamedTemporaryFile(
            suffix=".md", mode="w", delete=False, encoding="utf-8",
        )
        doc.write("| `scripts/foo.py` | Purpose | `bar()` :42, `baz()` :99 |\n")
        doc.close()
        refs = extract_refs(Path(doc.name))
        Path(doc.name).unlink()
        self.assertEqual(len(refs), 2)
        self.assertEqual(refs[0], ("scripts/foo.py", "bar", 42, 1))
        self.assertEqual(refs[1], ("scripts/foo.py", "baz", 99, 1))

    def test_inline_pattern(self):
        doc = tempfile.NamedTemporaryFile(
            suffix=".md", mode="w", delete=False, encoding="utf-8",
        )
        doc.write("See `scripts/foo.py:10` (my_func)\n")
        doc.close()
        refs = extract_refs(Path(doc.name))
        Path(doc.name).unlink()
        self.assertTrue(any(r[1] == "my_func" and r[2] == 10 for r in refs))


class TestFindSymbolLine(unittest.TestCase):
    """P5-39: find_symbol_line locates definitions."""
    ...
    def test_finds_def(self):
        ...
        f.write("# header\n\ndef my_func():\n    pass\n")
        f.close()
        self.assertEqual(find_symbol_line(Path(f.name), "my_func"), 3)
        Path(f.name).unlink()
    ...
    def test_finds_class(self):
        ...
        f.close()
        self.assertEqual(find_symbol_line(Path(f.name), "MyClass"), 1)
        Path(f.name).unlink()
    ...
    def test_returns_none_for_missing_symbol(self):
        ...
        f.write("x = 1\n")
        f.close()
        self.assertIsNone(find_symbol_line(Path(f.name), "nonexistent"))
        Path(f.name).unlink()

    def test_nonexistent_file_returns_none(self):
        self.assertIsNone(find_symbol_line(Path("/no/such/file.py"), "foo"))


class TestVerifyRef(unittest.TestCase):
    """P5-39: verify_ref detects stale vs correct refs."""
    ...
    def test_stale_ref_fails(self):
        """A reference far from actual line fails."""
        passed, msg = verify_ref(
            "scripts/validate_config.py", "parse_simple_toml", 200, 1,
        )
